...
- `GET /recipes` accepts five optional query parameters:
  ...
  - `tags` — comma-separated tag list, **OR** semantics among themselves (matches Quick Wins' existing frontend rule: a recipe matches if it has *any* of the given tags), **AND**-ed with `q` if both are present.
...
- The list page keeps the same toolbar (search input, tag chips, sort select + direction button) Quick Wins already built — only what happens *after* a change fires is different: instead of recomputing a local `computed()`, the component debounces and issues a new `GET /recipes` request.
...
- Quick Wins Part 5's "no recipes at all" vs. "no recipes match the filter" empty-state distinction can no longer be answered by comparing a full in-memory array's length before and after filtering (there is no full in-memory array). Reframe the check on *whether a filter is currently active* instead of *whether the database is empty*: if `q`/`tags` are both empty and the page comes back with zero content, show the existing "no recipes yet" CTA; if `q`/`tags` are non-empty and the page is empty, show "no recipes match your search" + "Clear filters". This is a strictly simpler condition than before (no need to compare two arrays) and produces the same user-visible behavior in every case that matters.
